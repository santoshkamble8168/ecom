...
- Current Context: **Sprints 0–16 complete.** Sprint 17 and automated test expansion remain **on hold**. Shopper campaign landings live at `/campaign/{slug}` (CMS campaign pages redirect from `/pages/{slug}` and keep UTM params). Admin attaches campaign products/collections via product/collection search, not raw SKU/UUID typing.
